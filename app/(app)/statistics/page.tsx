  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-text-primary text-2xl font-bold">Statistikk</h1>
        {isLoading && <span className="text-accent-muted text-sm">Laster…</span>}
      </div>
      ...
      <div className="rounded-xl bg-bg-card border border-border-teal p-4">
        <div className="flex items-center justify-between mb-4">
          <p className="text-text-primary font-semibold">Øvelsesfremgang</p>
          <Button variant="secondary" size="sm" onClick={() => setPickerOpen(true)}>
            {selectedExercise ? selectedExercise.name : 'Velg øvelse'}
          </Button>
        </div>
        {selectedExercise ? (
          <ExerciseProgressionChart data={progressionData} exerciseName={selectedExercise.name} />
        ) : (
          <p className="text-accent-muted text-sm py-8 text-center">Velg en øvelse for å se fremgang</p>
        )}
      </div>
